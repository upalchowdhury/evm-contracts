...
abstract contract RegistryClient is
  Initializable,
  ReentrancyGuard,
  Pausable
{
constructor(address _registry) {
    RegistryClient__initialize(_registry);
  }
  ...
  function RegistryClient__initialize(address _registry)
    internal
    initializer
  {
   ...
   * @param _role Role as defined in appLibrary
   */
  ...
  function pause() external virtual isAuthorized(appLib.PANIC_ROLE) {
    super._pause();
  }

  function unpause() external virtual isAuthorized(appLib.GUARDIAN_ROLE) {
    super._unpause();
  }
  ...
  function rescueTokens(address[] calldata _tokens, uint256[] memory _amounts)
    public
    whenPaused
    isAuthorized(appLib.GUARDIAN_ROLE)
  {
